...
| `error_msg` | 错误信息 | 从 Handler 返回错误获取 |
...
```go
// 在路由中使用中间件
func registerUserRoutes(r *echo.Echo, ctx *RouterContext) {
...
```go
// 在 Controller 中自定义
func (ctrl *UserController) ResetPassword(c *echo.Context) {
    // 设置自定义标题
...
func (ctrl *RoleController) AssignPermissions(c *echo.Context) {
    // 设置自定义信息
...
```go
func SetupRouter(container *container.Container) *echo.Echo {
    r := echo.New()
...
```go
// Controller 中自定义
func (ctrl *RoleController) AssignMenus(c *echo.Context) {
