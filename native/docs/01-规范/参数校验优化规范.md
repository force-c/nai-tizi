...
// 2. Controller 中使用
func (c *AttachmentController) Upload(ctx *echo.Context) {
...
// 2. Controller 中使用
func (c *UserController) List(ctx *echo.Context) {
...
// 2. Controller 中使用
func (c *UserController) GetById(ctx *echo.Context) {
...
| 方法 | 当前问题 | 优化方案 |
|------|---------|---------|
| UploadAttachment | 手动解析 Form 参数，Service 层重复定义 UploadRequest | 定义 `request.UploadAttachmentRequest`，使用 `Bind` |
